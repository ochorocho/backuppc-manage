...
if [ "$INSTALL" = "YES" ]; then
    ...
    grep -i centos < /etc/redhat-release && sudo "$PM" -y install epel-release
		sudo "$PM" $CONFIRM install make gcc libacl-devel httpd-tools curl perl samba-client rrdtool rsync par2cmdline tar perl-App-cpanminus iputils openssh-clients bzip2 hostname
	fi
